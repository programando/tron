 ...
 public  function  Buscar_datos_x_Idtercero_Presenta( $IdTercero = 0 ){
  ...
  public function nuevo_usuario( $presentado_por_amigo = 0){
      if ( $presentado_por_amigo == 0 ) {
        ...
        $this->View->codigoterceropresenta='';
      }
      if ( $presentado_por_amigo == 1 ) {
        $this->View->codigoterceropresenta = Session::Get('codigousuario');
        Session::Set('codigoterceropresenta', $this->View->codigoterceropresenta );
      }
...
public function  Registro_Nuevo_Usuario_Validaciones ( $Datos ){
  $this->codigo_usuario_generado = '';
  ...
  if  ( $es_cliente == FALSE && $es_empresario == FALSE){
        $Texto_Respuesta =  $Texto_Respuesta . 'Debe indicar el tipo de registro que desea crear : <strong>Cliente</strong> o <strong>Empresario TRON</strong>.<br>';
  }
   ...
   if ( strlen($identificacion) == 0){
    ...
    if ( $es_e_mail == FALSE ){
      $Texto_Respuesta =  $Texto_Respuesta . 'La dirección de correo electrónico no puede esta vacía y debe tener un formato válido.<br>';
    }

   if ( strlen($passwordusuario) == 0){
        $Texto_Respuesta =  $Texto_Respuesta . 'Debe registrar una contraseña para ingresar a su cuenta.<br>';
    }
  ...
  // GENERACIÓN CÓDIGO DE USUARIO. SI NO ES EMPRESA ->Generar_Codigo_Usuario .... SINO Generar_Codigo_Emmpresa
    if ( strlen( $Texto_Respuesta ) == 0 ){
      if ( $idtpidentificacion != '31'  )  {
        $Datos_Nuevo_Codigo      = $this->Terceros->Generar_Codigo_Usuario($pnombre ,$papellido, $dianacimiento, $mesnacimiento);
        $this->codigo_usuario_generado = $Datos_Nuevo_Codigo [0]['codigo_generado'];
      }
      if ( $idtpidentificacion == '31'   ) {
        $Datos_Nuevo_Codigo      = $this->Terceros->Generar_Codigo_Emmpresa( $razonsocial);
        $this->codigo_usuario_generado = $Datos_Nuevo_Codigo [0]['codigo_generado'];
...
public function Registro_Nuevo_Usuario() {
 ...
 /// SI NADIE LO  PRESENTA, DEJÓ EN BLANCO DEBO ASIGNARLO
     ...
     {
        $codigousuario                                  = $this->codigo_usuario_generado ;
        $codautorizacionmenoredad                       = '';
...
//*** FIN  EDICION REGISTRO  Registro_Actualizar_Datos_Basicos
 ...
 public function Registro_Buscar_Por_Codigo( $codigousuario_presenta='', $salida_jquery=TRUE){
    ...
    /**  MARZO 17 DE 2015
      ...
      */
    Session::Iniciar_Variable('Consulto_Cpras_Tron_Mes_Actual'    , TRUE );
    if (  Session::Get('Consulto_Cpras_Tron_Mes_Actual') == TRUE ){
      ...
      if ($compras_este_mes_tron  >= $compra_minima_productos_tron ||  $compras_este_mes_industiales >= $compra_minima_productos_industriales){
        Session::Set('cumple_condicion_cpras_tron_industial', TRUE);
          ...
          Session::Set('Consulto_Cpras_Tron_Mes_Actual', FALSE ) ; // PASA A FALSE PARA NO VOLVER A CONSULTAR ESTOS DATOS.
      }// Fin Condicion
